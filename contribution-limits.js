...
const CONTRIBUTION_LIMITS = {
    ...
    FHSA: {
        ...
        introductionYear: 2023, // FHSA was introduced in 2023
 ...
 * @param {number} unusedRoomFromPreviousYears - Unused room from previous years (if manually specified, otherwise will be auto-calculated)
 * @returns {number} Available FHSA contribution room
 */
function calculateFHSARoom(age, isFirstTimeBuyer, currentYearContributions = 0, lifetimeContributions = 0, unusedRoomFromPreviousYears = null) {
    // FHSA eligibility: must be between 18 and 71
    if (age < 18 || age >= 71) {
        return 0;
    }
    if (!isFirstTimeBuyer) {
    ...
    const currentYear = 2025;
    const introductionYear = CONTRIBUTION_LIMITS.FHSA.introductionYear;
    
    // Auto-calculate unused room from previous years if not manually specified or if 0
    let calculatedUnusedRoom = unusedRoomFromPreviousYears;
    if (calculatedUnusedRoom === null || calculatedUnusedRoom === undefined || calculatedUnusedRoom === 0) {
        // Calculate how many years they've been eligible (since FHSA introduction in 2023)
        // They must have been 18+ and a first-time buyer
        const eligibleStartYear = Math.max(introductionYear, currentYear - (age - 18));
        
        // Calculate total room they've had in previous years (before current year)
        let totalPreviousRoom = 0;
        for (let year = eligibleStartYear; year < currentYear; year++) {
            if (year >= introductionYear) {
                // Check if they were 18+ in that year
                const ageInYear = age - (currentYear - year);
                if (ageInYear >= 18) {
                    totalPreviousRoom += CONTRIBUTION_LIMITS.FHSA.annualLimit;
                }
            }
        }
        
        // Calculate what they contributed in previous years (excluding current year)
        const previousYearContributions = Math.max(0, lifetimeContributions - currentYearContributions);
        
        // Unused room = total previous room - previous year contributions
        calculatedUnusedRoom = Math.max(0, totalPreviousRoom - previousYearContributions);
        
        // Cap at maximum carry-forward ($8,000)
        calculatedUnusedRoom = Math.min(calculatedUnusedRoom, CONTRIBUTION_LIMITS.FHSA.maxCarryForward);
    } else {
        // Use the manually specified value, but cap it at max carry-forward
        calculatedUnusedRoom = Math.min(unusedRoomFromPreviousYears, CONTRIBUTION_LIMITS.FHSA.maxCarryForward);
    }
    ...
    const maxCarryForward = Math.min(CONTRIBUTION_LIMITS.FHSA.maxCarryForward, calculatedUnusedRoom);
