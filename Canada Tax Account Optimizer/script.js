...
function initializeEventListeners() {
    ...
    document.getElementById('amountToMove').addEventListener('input', handleInputChange);
...
function collectInputs() {
    ...
    const isFirstTimeBuyer = document.querySelector('input[name="homePurchase"]:checked').value === 'no';
...
function calculateRooms(inputs) {
    ...
    const rooms = {
        ...
        fhsa: calculateFHSARoom(
            ...
            inputs.fhsaContributions, // Assuming lifetime = current year for simplicity
            0 // Unused room from previous years (user can input this if needed)
        ),
...
function updateInputSummary(inputs, rooms) {
    ...
    const summaryItems = [
        ...
        { label: 'Amount to Move', id: 'amountToMove', type: 'number', value: inputs.amountToMove },
        { label: 'First-Time Home Buyer', id: 'homePurchase', type: 'radio', value: inputs.isFirstTimeBuyer ? 'no' : 'yes' },
        ...
        { label: '2024 Income', id: 'income2024', type: 'number', value: inputs.income2024 },
    ...
    summaryItems.forEach(item => {
        ...
        if (item.type === 'select') {
            ...
            html += `<label class="radio-label"><input type="radio" name="summary_${item.id}" value="no" ${item.value === 'no' ? 'checked' : ''}> No (First-time buyer)</label>`;
            html += `<label class="radio-label"><input type="radio" name="summary_${item.id}" value="yes" ${item.value === 'yes' ? 'checked' : ''}> Yes</label>`;
            html += `</div>`;
        } else {
            html += `<input type="${item.type}" id="summary_${item.id}" class="summary-input" value="${item.value || ''}" ${item.type === 'number' ? 'step="100"' : ''}>`;
        }
